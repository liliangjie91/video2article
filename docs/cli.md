...
- `integrate` — 读取 insights 中的 web_search 数据，去重排序后输出 `search_references.json`
- `link` — 读取文章 + 搜索结果，后处理插入 `[词](url)` 内联引用链接，覆盖原文件
...
- `--search` — 开启联网搜索，基于大纲生成查询 → 搜索结果整合 → 文章后处理插入引用链接（仅 article / batch）
