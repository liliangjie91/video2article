...
python main.py article <url> --search          # 开启联网搜索（消耗 API credit）
python main.py article <url> --deliver         # 生成后自动投送默认渠道
...
python main.py debug search       <query>              # 单次搜索测试
python main.py debug search       <query> -e brave      # 指定引擎
python main.py debug search       <query> -l 10         # 指定结果数
python main.py debug integrate    <03_insights.json>    # 整合搜索结果
python main.py debug link         <05_article.md> <search_references.json>  # 插入链接
```

每个 stage 输出到 `0N_name.ext`，输入前一步的输出文件。

搜索与链接调试：
- `search` — 调用搜索引擎返回原始结果，用于测试查询效果
- `integrate` — 读取 insights 中的 web_search 数据，LLM 去重排序后输出 `search_references.json`
- `link` — 读取文章 + 搜索结果，后处理插入 `[词](url)` 内联引用链接（跨段落同词不重复），输出 `05_article_link.md`
...
投递 `05_article.md` 时，若同目录存在 `05_article_link.md` 则自动优先投递后者。
...
python main.py batch <url1> --search                        # 开启联网搜索
```
...
- `--simple` — 快速产出模式（仅 article / batch）
- `--search` — 开启联网搜索，insights 生成搜索词 → outline 去重筛选 → 检索 → 文章后处理插入引用链接，输出 `05_article_link.md`（仅 article / batch）
- `--deliver` — 文章生成后自动投送到默认渠道（仅 article）
