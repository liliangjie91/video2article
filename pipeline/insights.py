...
def _build_segment_prompt(segment: dict, text: str) -> str:
    topic = segment.get("topic", "未命名")
    time_range = segment.get("time_range", "未知时间")
...
def run(preprocessed_path: str, structure_path: str, output_dir: str, tier: str = "best") -> str:
    ...
    # Simple text splitting: use the full text for context but
    # focus analysis on each segment's time range
    for i, seg in enumerate(segments):
        ...
        prompt_parts = [
            ...
            "",
            "当前需要分析的段落：",
            _build_segment_prompt(seg, full_text),
            "",
