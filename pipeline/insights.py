...
### 6. 搜索建议
生成两类搜索查询词，用于后续查找延伸阅读资料：

**概念词（concept_queries）**：提取本段中值得查百科解释的关键实体词——技术术语、人名、事件名、理论名等。每个词 2-8 个字，生成 2-4 个。如"GDP平减指数"、"Transformer架构"。

**探索查询（explore_queries）**：针对本段中有深度挖掘空间的论点或背景，生成短语或短问句。每句 8-20 个字，生成 2-4 个。如"GDP 计算方法 局限性"、"Transformer 相比 RNN 的核心优势"。

两类查询都必须聚焦于本段特有内容，不要生成通用宽泛的词。
...
```json
{
  ...
  "concept_queries": ["术语1", "术语2", "术语3"],
  "explore_queries": ["探索查询短语1", "探索查询短语2"]
}
```
...
def run(structure_path: str, output_dir: str, tier: str = "best") -> str:
    ...
    for seg in segments:
        ...
        merged["insight"] = {
            ...
            "concept_queries": matched.get("concept_queries", []),
            "explore_queries": matched.get("explore_queries", []),
        }
