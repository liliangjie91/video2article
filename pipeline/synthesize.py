...
def _build_structure_guide(word_count: int | None, key_points: list[str]) -> str:
    """Build structured writing guide: word count range + paragraph layout."""
    wc = word_count or 500
    n = max(len(key_points), 1)
    margin = max(int(wc * 0.15), 50)
    low, high = wc - margin, wc + margin

    intro = int(wc * 0.18)
    body = int(wc * 0.64)
    conclusion = wc - intro - body
    per_point = int(body / n)

    lines = [f"字数目标：{wc} 字（允许范围 {low}-{high} 字）"]
    lines.append("建议结构：")
    lines.append(f"- 开篇 1-2 句引出核心观点（约 {intro} 字）")
    if n <= 1:
        lines.append(f"- 中间 3-5 句深入展开论述（约 {body} 字）")
    else:
        lines.append(f"- 围绕 {n} 个关键点分别展开，每点约 {per_point} 字（共约 {body} 字）")
    lines.append(f"- 结尾 1-2 句收束过渡（约 {conclusion} 字）")
    return "\n".join(lines)
...
def run(insights_path: str, outline_path: str, output_dir: str, tier: str = "best") -> str:
    ...
    for i, oseg in enumerate(outline_segments):
        ...
        prompt_parts = [
            ...
            _build_structure_guide(oseg.get('word_count_estimate'), oseg.get('key_points', [])),
