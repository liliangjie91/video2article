...
5. 每个大纲段落的 `queries` 必须包含且仅包含 2 个查询词：第一个必须是**概念词（concept_query）**，第二个必须是**探索查询（explore_query）**
   - 概念词 = 实体术语（技术名词、人名、事件名等），2-8 个字
   - 探索查询 = 短语或短问句，8-20 个字
   - 直接从输入材料中每段附带的「搜索建议」中选择，不要自行发明
   - 探索查询必须不同于概念词，也**不得包含**概念词
6. 每个大纲段落的两个查询必须指向不同方向，不能同义或近似

## 输出格式（必须严格 JSON）
```json
{
  "meta": {
    "title": "建议的文章标题（10-15个汉字，不要有特殊字符、空格、英文引号，吸引眼球）",
  ...
  "outline": [
    {
      ...
      "queries": ["concept_query", "explore_query"],
      "sources": [
...
def run(insights_path: str, output_dir: str, tier: str = "best",
    ...
    for seg in insights_data.get("segments", []):
        ...
        prompt_parts.append(
            ...
            f"概念词建议：{'、'.join(insight.get('concept_queries', []))}\n"
            f"探索查询建议：{'、'.join(insight.get('explore_queries', []))}"
        )
    ...
    # Collect per-segment queries into a top-level deduplicated list
    ...
    outline["search_queries"] = all_queries
